...
class MerkleX extends EventEmitter {
  ...
  newOrder(details) {
    ...
    const p = this.markets().then(
      ...
      .then(market => {
        ...
        request.quantity = Big(request.quantity).mul(Big(10).pow(market.base_decimals)).toFixed(0);
        request.price = Big(request.price).mul(Big(10).pow(market.price_decimals)).toFixed(0);
  ...
  _getNextOrderToken() {
    let next_token = this._next_order_token++;
    return next_token;
  }
