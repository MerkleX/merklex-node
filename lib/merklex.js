...
class MerkleX extends EventEmitter {
  constructor(settings) {
    ...
    this._gateway.on('connect', () => {
      this.assets().then(() => this.markets()).then(() => {
        /* auth on connection */
        this._accessKey()
          .then(access_key => {
            this._gateway.send({
  ...
  _handleOrderResends() {
    const now = Date.now();
    Object.keys(this._pending_orders).forEach(order_token => {
      const [ accept, reject, request, ts ] = this._pending_orders[order_token];
      if (now - ts > 100) {
        this._send(request);
        this._pending_orders[order_token][3] = now;
      }
    });
  }

  _handleReport(report) {
    this.emit('report', MapReport(this._assets, this._markets, report));
    ...
    switch (report.header.type_id) {
      ...
      case Reports.Types.MARKET_READY: {
        this._prepared_markets[this._markets.find(m =>
      ...
      case Reports.Types.ORDER_ACCEPTED: {
        const cb = this._pending_orders[report.order_token];
        if (cb) {
          delete this._pending_orders[report.order_token];
          cb[0](report);
        }
        break;
      }
      case Reports.Types.ORDER_REJECTED: {
        ...
        if (cb) {
          ...
          err.order_token = report.order_token;
          err.report = report;
          cb[1](err);
        }
        break;
  ...
  newOrder(details) {
    if (!details.market && (!details.quote_asset_id || !details.base_asset_id)) {
      return Promise.reject(new Error('missing market'));
    }

    if (!details.quantity) {
      return Promise.reject(new Error('missing quantity'));
    }

    if (!details.price) {
      return Promise.reject(new Error('missing price'));
    }

    if (!details.is_buy && details.is_buy !== false) {
      return Promise.reject(new Error('missing is_buy'));
    }
    ...
    return this.markets().then(
      markets => markets.find(m =>
        m.symbol === details.market
        || (m.quote_asset_id === details.quote_asset_id && m.base_asset_id === decimals.base_asset_id)
      ))
      .then(market => {
        if (!this._prepared_markets[market.symbol]) {
          this._prepareMarket(market);
          throw new Error('market not prepared');
        }

        request.quantity = Big(request.quantity).mul(Big(10).pow(market.base_decimals));
        request.price = Big(request.price).mul(Big(10).pow(market.price_decimals));
        request.quote_asset_id = market.quote_asset_id;
        request.base_asset_id = market.base_asset_id;
        request.order_token = this._getNextOrderToken();
        request.replace_order_token = request.replace_order_token || 0;

        return new Promise((resolve, reject) => {
          this._pending_orders[request.order_token] = [ resolve, reject, request, Date.now() ];
          this._send(request);
        });
      });
  }
  ...
  _getNextOrderToken() {
    let next_token = this.next_order_token++;
    return next_token;
  }
  ...
  updateTradingLimit(data) {
    return this._accessKey().then(key => {
      const auth_header = `addr_code ${key} ${this._user_id}`;

      return this.markets()
        .then(markets => markets.find(m => m.quote_asset_id === +data.quote_asset_id && m.base_asset_id === +data.base_asset_id))
        .then(market => {
          return requests
        ...
        .then(trading_limit => {
          const updated_version = {
            ...data,
            user_id: this._user_id,
            limit_version: '' + trading_limit.next_limit_version,
          };
          const data_hash = this._tradingLimitHash(updated_version).substr(2);
          updated_version.signature = this._sigToStr(ecsign(Buffer.from(data_hash, 'hex'), this._priv_key));

          return requests.post(this._rest_uri + '/v1/trade/limit')
            .set('Authorization', auth_header)
            .send(updated_version)
            .then(data => {
